...
/datum/disease/advance/proc/AssignProperties(var/list/properties = list())
	...
	if(properties && properties.len)
		...
		SetSpread(clamp2(properties["transmittable"] - symptoms.len, BLOOD, AIRBORNE))
		permeability_mod = max(Ceiling(0.4 * properties["transmittable"]), 1)
		cure_chance = 15 - clamp2(properties["resistance"], -5, 5) // can be between 10 and 20
...
/datum/disease/advance/proc/GenerateCure(var/list/properties = list())
	if(properties && properties.len)
		var/res = clamp2(properties["resistance"] - (symptoms.len / 2), 1, advance_cures.len)
		//world << "Res = [res]"
