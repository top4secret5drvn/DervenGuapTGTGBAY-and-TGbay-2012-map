...
/obj/machinery/computer/telescience/proc/doteleport(mob/user)
	...
	if(telepad)
		...
		var/truePower = clamp2(power + power_off, 1, 1000)
		var/trueRotation = rotation + rotation_off
		var/trueAngle = clamp2(angle + angle_off, 1, 90)

		var/datum/projectile_data/proj_data = projectile_trajectory(telepad.x, telepad.y, trueRotation, trueAngle, truePower)
		last_tele_data = proj_data

		var/trueX = clamp2(round(proj_data.dest_x, 1), 1, world.maxx)
		var/trueY = clamp2(round(proj_data.dest_y, 1), 1, world.maxy)
...
/obj/machinery/computer/telescience/Topic(href, href_list)
	...
	if(href_list["setrotation"])
		...
		if(..()) // Check after we input a value, as they could've moved after they entered something
			return
		rotation = clamp2(new_rot, -900, 900)
	...
	if(href_list["setangle"])
		...
		if(..())
			return
		angle = clamp2(round(new_angle, 0.1), 1, 9999)
